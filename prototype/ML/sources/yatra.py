...
class YatraPlaywrightSource(BaseSource):
    name = "Yatra"
    SEARCH_URL_TEMPLATE = "https://www.yatra.com/"
    ...
    async def fetch_fares(self, origin: str, dest: str, t_window: str, offset_days: int) -> list[dict]:
        ...
        try:
            ...
            await context.close()
            raise RuntimeError(f"Yatra navigation failed: {e}")
        ...
        print(f"[INFO] Yatra API payload not intercepted for {origin}-{dest}.")
        return []
