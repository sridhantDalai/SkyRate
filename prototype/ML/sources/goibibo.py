...
class GoibiboPlaywrightSource(BaseSource):
    name = "Goibibo"
    SEARCH_URL_TEMPLATE = "https://www.goibibo.com/"
    ...
    async def fetch_fares(self, origin: str, dest: str, t_window: str, offset_days: int) -> list[dict]:
        ...
        try:
            ...
            await context.close()
            raise RuntimeError(f"Goibibo navigation failed: {e}")
        ...
        print(f"[INFO] Goibibo API payload not intercepted for {origin}-{dest}.")
        return []
