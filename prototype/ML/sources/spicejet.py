...
class SpiceJetPlaywrightSource(BaseSource):
    name = "SpiceJet"
    SEARCH_URL_TEMPLATE = "https://www.spicejet.com/"
    ...
    async def fetch_fares(self, origin: str, dest: str, t_window: str, offset_days: int) -> list[dict]:
        ...
        try:
            ...
            await context.close()
            raise RuntimeError(f"SpiceJet navigation failed: {e}")
        ...
        print(f"[INFO] SpiceJet API payload not intercepted for {origin}-{dest}.")
        return []
