...
class AkasaAirPlaywrightSource(BaseSource):
    name = "AkasaAir"
    SEARCH_URL_TEMPLATE = "https://www.akasaair.com/"
    ...
    async def fetch_fares(self, origin: str, dest: str, t_window: str, offset_days: int) -> list[dict]:
        ...
        try:
            ...
            await context.close()
            raise RuntimeError(f"Akasa Air navigation failed: {e}")
        ...
        print(f"[INFO] Akasa Air API payload not intercepted for {origin}-{dest}.")
        return []
