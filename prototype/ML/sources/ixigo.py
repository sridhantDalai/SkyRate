...
class IxigoPlaywrightSource(BaseSource):
    name = "Ixigo"
    SEARCH_URL_TEMPLATE = "https://www.ixigo.com/"
    ...
    async def fetch_fares(self, origin: str, dest: str, t_window: str, offset_days: int) -> list[dict]:
        ...
        try:
            ...
            await context.close()
            raise RuntimeError(f"Ixigo navigation failed: {e}")
        ...
        print(f"[INFO] Ixigo API payload not intercepted for {origin}-{dest}.")
        return []
