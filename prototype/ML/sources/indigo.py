...
class IndiGoPlaywrightSource(BaseSource):
    name = "IndiGo"

    SEARCH_URL_TEMPLATE = "https://www.goindigo.in/"
    ...
    async def fetch_fares(self, origin: str, dest: str, t_window: str, offset_days: int) -> list[dict]:
        ...
        async def on_response(response):
            if "api/flights" in response.url or "Search" in response.url:
                try:
        ...
        try:
            # IndiGo uses a SPA, we navigate to home, and normally would script Playwright to fill the search boxes
            # For this architecture stub, we just load the page and wait for the payloads.
            ...
            await context.close()
            raise RuntimeError(f"IndiGo navigation failed: {e}")
        ...
        print(f"[INFO] IndiGo API payload not intercepted for {origin}-{dest}.")
        return []

    def normalize(self, raw_payload: dict, origin: str, dest: str, t_window: str) -> list[dict]:
        records = []
        try:
            # Structural parsing stub for IndiGo JSON
            ...
            print(f"[ERROR] Failed to normalize IndiGo payload: {e}")
        return records
